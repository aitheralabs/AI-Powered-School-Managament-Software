...
class AcademicYearService extends baseService_1.BaseService {
    async createAcademicYear(academicYearData) {
        ...
        const existingYear = await this.executeQuery('SELECT id FROM academic_years WHERE name = $1 AND school_id = $2', [academicYearData.name, schoolId]);
        ...
        const result = await this.executeTransaction(async (client) => {
            if (academicYearData.isActive) {
                await client.query('UPDATE academic_years SET is_active = false WHERE is_active = true AND school_id = $1', [schoolId]);
            }
            const sequentialId = await this.generateSequentialId('academic_years');
            const result = await client.query(`INSERT INTO academic_years (name, start_date, end_date, is_active, alt_id, school_id)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id, alt_id, name, start_date, end_date, is_active, created_at, updated_at`, [academicYearData.name, academicYearData.startDate, academicYearData.endDate,
                academicYearData.isActive || false, sequentialId, schoolId]);
            return this.transformAcademicYearResponse(result.rows[0]);
        });
        await cacheService_1.cacheService.delPattern(`${cacheService_1.CacheKeys.ACADEMIC_YEAR}:${schoolId}:*`);
        return result;
    }
    async getAcademicYears(req) {
       ...
       LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`, [...queryParams, limit, offset]);
        return {
            academicYears: result.rows.map((year) => this.transformAcademicYearResponse(year)),
    ...
    async getAcademicYearById(id) {
        ...
        const semestersResult = await this.executeQuery('SELECT id, alt_id, name, start_date, end_date, is_active, created_at, updated_at FROM semesters WHERE academic_year_id = $1 AND school_id = $2 ORDER BY start_date', [academicYear.id, schoolId]);
        return {
            ...this.transformAcademicYearResponse(academicYear),
            semesters: semestersResult.rows.map((s) => ({
    ...
    async updateAcademicYear(id, updateData) {
        ...
        return await this.executeTransaction(async (client) => {
            if (updateData.isActive) {
                await client.query('UPDATE academic_years SET is_active = false WHERE is_active = true AND id != $1 AND school_id = $2', [actualYearId, schoolId]);
            }
    ...
    async deleteAcademicYear(id) {
        ...
        const dep = await this.executeQuery(`SELECT
         ...
         (SELECT COUNT(*) FROM fee_categories WHERE academic_year_id = $1 AND school_id = $2)::INT AS fee_categories_count`, [actualYearId, schoolId]);
        const d = dep.rows[0];
        if (d.classes_count + d.semesters_count + d.fee_categories_count > 0) {
            throw new errorHandler_1.AppError(`Cannot delete. Has ${d.classes_count} classes, ${d.semesters_count} semesters, ${d.fee_categories_count} fee categories.`, 409);
        }
        await this.executeQuery('DELETE FROM academic_years WHERE id = $1 AND school_id = $2', [actualYearId, schoolId]);
        return { success: true };
    }
    async getActiveAcademicYear() {
        const schoolId = this.requireSchool();
        return await cacheService_1.cacheService.cacheQuery(`${cacheService_1.CacheKeys.ACADEMIC_YEAR_ACTIVE}:${schoolId}`, async () => {
            const result = await this.executeQuery('SELECT id, alt_id, name, start_date, end_date, is_active, created_at, updated_at FROM academic_years WHERE is_active = true AND school_id = $1 LIMIT 1', [schoolId]);
