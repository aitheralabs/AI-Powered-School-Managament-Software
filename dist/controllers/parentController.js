...
exports.updateParentStudentRelationship = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const result = await svc.executeQuery(`UPDATE student_parents SET relationship_type = COALESCE($1, relationship_type), is_primary = COALESCE($2, is_primary), updated_at = CURRENT_TIMESTAMP
     WHERE id = $3
     RETURNING *`, [
...
exports.removeParentStudentRelationship = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { relationshipId } = req.params;
    const svc = parentService.forSchool(req.schoolId);
    await svc.executeQuery("DELETE FROM student_parents WHERE id = $1", [relationshipId]);
