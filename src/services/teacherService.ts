...
export class TeacherService extends BaseService {
  async createTeacher(teacherData: CreateTeacher) {
    ...
    const existingEmployee = await this.executeQuery(
      ...
      [teacherData.employeeId, schoolId]
    );
    ...
    return await this.executeTransaction(async (client) => {
      ...
      const userResult = await client.query(
        ...
        [teacherData.firstName, teacherData.lastName, teacherData.email, passwordHash,
      ...
      const teacherResult = await client.query(
        `INSERT INTO teachers (user_id, employee_id, qualification, experience_years, specialization, joining_date, salary, alt_id, school_id)
        ...
        [user.id, teacherData.employeeId, teacherData.qualification || null,
  ...
  async getTeachers(req: any) {
    ...
    if (isActive !== undefined) { whereClause += ` AND t.is_active = $${queryParams.length + 1}`; queryParams.push(isActive === 'true'); }
    if (search) {
    ...
    if (specialization) { whereClause += ` AND t.specialization ILIKE $${queryParams.length + 1}`; queryParams.push(`%${specialization}%`); }

    const countResult = await this.executeQuery(
      `SELECT COUNT(*) FROM teachers t JOIN users u ON t.user_id = u.id ${whereClause}`, queryParams
    );
  ...
  async getTeacherById(id: string) {
    ...
    const result = await this.executeQuery(
      `SELECT t.id, t.alt_id, t.user_id, t.employee_id, t.qualification, t.experience_years,
              t.specialization, t.joining_date, t.salary, t.is_active, t.created_at, t.updated_at,
              u.first_name, u.last_name, u.email, u.phone, u.date_of_birth, u.address
       FROM teachers t JOIN users u ON t.user_id = u.id ${idFilter}`,
      [id, schoolId]
    );
    if (result.rows.length === 0) throw new AppError('Teacher not found', 404);
    ...
    const subjectsResult = await this.executeQuery(
      `SELECT s.id, s.name, s.code, s.description, s.credit_hours
       ...
       WHERE ts.teacher_id = $1 AND s.is_active = true ORDER BY s.name`,
      [teacher.id, schoolId]
    );

    const classesResult = await this.executeQuery(
      `SELECT c.id, c.name, c.grade, c.section, c.capacity, c.room, ay.name as academic_year_name
       FROM classes c
    ...
    return {
      ...
      subjects: subjectsResult.rows.map((s: any) => ({ id: s.id, name: s.name, code: s.code, creditHours: s.credit_hours })),
      classes: classesResult.rows.map((c: any) => ({ id: c.id, name: c.name, grade: c.grade, section: c.section, academicYearName: c.academic_year_name })),
    };
  }
    ...
    return await this.executeTransaction(async (client) => {
      ...
      if (Object.keys(teacherUpdateData).length > 0) {
        ...
        teacherResult = await client.query(
          'SELECT id, alt_id, user_id, employee_id, qualification, experience_years, specialization, joining_date, salary, is_active, created_at, updated_at FROM teachers WHERE id = $1',
          [teacherId]
        );
      }
  ...
  async deleteTeacher(id: string) {
    ...
    const assignmentsCheck = await this.executeQuery(
      'SELECT COUNT(*) FROM classes WHERE teacher_id = $1 AND is_active = true AND school_id = $2',
      [teacherId, schoolId]
    );
    if (parseInt(assignmentsCheck.rows[0].count) > 0) {
      throw new AppError('Cannot deactivate teacher with active class assignments. Reassign classes first.', 409);
    }
