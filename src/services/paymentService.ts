...
export class PaymentService extends BaseService {
  async recordPayment(paymentData: CreatePayment, processedBy: string) {
    ...
    const studentFeeResult = await this.executeQuery(
      ...
      [paymentData.studentFeeId, schoolId]
    );

    if (studentFeeResult.rows.length === 0) throw new AppError('Student fee not found', 404);

    const studentFee = studentFeeResult.rows[0];
    if (studentFee.status === 'paid') throw new AppError('This fee has already been fully paid', 400);

    const paidAmountResult = await this.executeQuery(
      'SELECT COALESCE(SUM(amount), 0) as total_paid FROM payments WHERE student_fee_id = $1 AND school_id = $2',
      [paymentData.studentFeeId, schoolId]
    );
    ...
    if (paymentData.amount > pendingAmount) throw new AppError(`Payment amount cannot exceed pending amount of ${pendingAmount}`, 400);

    return await this.executeTransaction(async (client) => {
      const receiptNumber = await this.generateReceiptNumber(schoolId);

      const paymentResult = await client.query(
        `INSERT INTO payments (student_fee_id, amount, payment_date, payment_method, transaction_id, receipt_number, processed_by, school_id)
      ...
      await client.query(
        'UPDATE student_fees SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND school_id = $3',
        [newStatus, paymentData.studentFeeId, schoolId]
      );
  ...
  private async executePaymentsQuery(req: any) {
    ...
    if (paymentMethod) { whereClause += ` AND p.payment_method = $${queryParams.length + 1}`; queryParams.push(paymentMethod); }
    ...
    const countResult = await this.executeQuery(
      `SELECT COUNT(*) FROM payments p JOIN student_fees sf ON p.student_fee_id = sf.id JOIN students s ON sf.student_id = s.id ${whereClause}`,
      queryParams
    );
  ...
  async reversePayment(paymentId: string, reason: string, reversedBy: string) {
    const schoolId = this.requireSchool();
    const paymentResult = await this.executeQuery(
      `SELECT p.id, p.amount, p.student_fee_id, p.receipt_number, sf.amount as fee_amount, sf.status as fee_status
       FROM payments p
       JOIN student_fees sf ON p.student_fee_id = sf.id
    ...
    return await this.executeTransaction(async (client) => {
      // Soft-delete by marking as reversed (add a notes/metadata column approach)
      await client.query(
        `DELETE FROM payments WHERE id = $1 AND school_id = $2`,
        [paymentId, schoolId]
      );

      // Recalculate new paid amount
  ...
  async getPaymentHistory(studentId: string) {
    const schoolId = this.requireSchool();
    const result = await this.executeQuery(
      `SELECT p.id, p.amount, p.payment_date, p.payment_method, p.receipt_number, p.transaction_id,
  ...
  async getPaymentReceipt(paymentId: string) {
    const schoolId = this.requireSchool();
    const result = await this.executeQuery(
      `SELECT p.id, p.amount, p.payment_date, p.payment_method, p.transaction_id, p.receipt_number,
              p.created_at, sf.amount as fee_amount, sf.status as fee_status,
  ...
  private async generateReceiptNumber(schoolId: string): Promise<string> {
    ...
    const countResult = await this.executeQuery(
      'SELECT COUNT(*) as count FROM payments WHERE DATE(created_at) = CURRENT_DATE AND school_id = $1',
      [schoolId]
    );
