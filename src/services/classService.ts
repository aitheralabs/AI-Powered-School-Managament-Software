...
export class ClassService extends BaseService {
  async createClass(classData: any) {
    ...
    return await this.executeTransaction(async (client) => {
      const academicYearExists = await client.query(
        'SELECT id, name FROM academic_years WHERE id = $1 AND school_id = $2',
        [classData.academicYearId, schoolId]
      );
      ...
      if (classData.teacherId) {
        const teacherExists = await client.query(
          "SELECT id FROM users WHERE id = $1 AND role = 'teacher' AND is_active = true AND school_id = $2",
          [classData.teacherId, schoolId]
        );
        if (teacherExists.rows.length === 0) throw new AppError('Teacher not found or inactive', 404);

        const existingAssignment = await client.query(
          'SELECT id, grade, section FROM classes WHERE teacher_id = $1 AND is_active = true AND school_id = $2',
          [classData.teacherId, schoolId]
        );
        if (existingAssignment.rows.length > 0) {
          const ec = existingAssignment.rows[0];
          throw new AppError(`Teacher is already assigned to class ${ec.grade}-${ec.section}`, 409);
        }
      }

      const existingClass = await client.query(
        'SELECT id FROM classes WHERE grade = $1 AND section = $2 AND academic_year_id = $3 AND school_id = $4',
        [classData.grade, classData.section, classData.academicYearId, schoolId]
      );
      ...
      if (newClass.teacher_id) {
        const teacherResult = await client.query(
          'SELECT first_name, last_name FROM users WHERE id = $1 AND school_id = $2',
          [newClass.teacher_id, schoolId]
        );
      ...
      await cacheService.delPattern(`${CacheKeys.CLASSES_ALL}:${schoolId}:*`);
  ...
  async getClasses(req: any) {
    ...
    const cacheKey = `${CacheKeys.CLASSES_ALL}:${schoolId}:${page}:${limit}:${isActive || 'all'}:${academicYearId || 'all'}:${grade || 'all'}:${teacherId || 'all'}`;

    return await cacheService.cacheQuery(cacheKey, async () => {
      let whereClause = 'WHERE c.school_id = $1';
      const queryParams: any[] = [schoolId];

      if (isActive !== undefined) { whereClause += ` AND c.is_active = $${queryParams.length + 1}`; queryParams.push(isActive === 'true'); }
      if (academicYearId) { whereClause += ` AND c.academic_year_id = $${queryParams.length + 1}`; queryParams.push(academicYearId); }
      if (grade) { whereClause += ` AND c.grade = $${queryParams.length + 1}`; queryParams.push(grade); }
      if (teacherId) { whereClause += ` AND c.teacher_id = $${queryParams.length + 1}`; queryParams.push(teacherId); }

      const countResult = await this.executeQuery(
        `SELECT COUNT(*) FROM classes c JOIN academic_years ay ON c.academic_year_id = ay.id ${whereClause}`,
        queryParams
      );
  ...
  async getClassById(id: string) {
    ...
    const academicYearResult = await this.executeQuery(
      'SELECT id, name FROM academic_years WHERE id = $1 AND school_id = $2',
      [classInfo.academic_year_id, schoolId]
    );
  ...
  async updateClass(id: string, updateData: any) {
    ...
    return await this.executeTransaction(async (client) => {
      if (updateData.teacherId) {
        const teacherExists = await client.query(
          "SELECT id FROM users WHERE id = $1 AND role = 'teacher' AND is_active = true AND school_id = $2",
          [updateData.teacherId, schoolId]
        );
        if (teacherExists.rows.length === 0) throw new AppError('Teacher not found or inactive', 404);

        const existingAssignment = await client.query(
          'SELECT id, grade, section FROM classes WHERE teacher_id = $1 AND is_active = true AND id != $2 AND school_id = $3',
          [updateData.teacherId, actualClassId, schoolId]
        );
        if (existingAssignment.rows.length > 0) {
          const ec = existingAssignment.rows[0];
          throw new AppError(`Teacher already assigned to class ${ec.grade}-${ec.section}`, 409);
        }
      }
  ...
  async deleteClass(id: string) {
    ...
    const studentsCheck = await this.executeQuery(
      'SELECT COUNT(*) FROM students WHERE class_id = $1 AND school_id = $2 AND is_active = true',
      [classInfo.id, schoolId]
    );
    if (parseInt(studentsCheck.rows[0].count) > 0) {
      throw new AppError('Cannot delete class with active students. Reassign or deactivate them first.', 409);
    }

    await this.executeQuery(
      'UPDATE classes SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND school_id = $2',
      [classInfo.id, schoolId]
    );
