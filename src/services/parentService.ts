...
export class ParentService extends BaseService {
  async createParent(parentData: CreateParent) {
    ...
    const existingUser = await this.executeQuery(
      ...
      [parentData.email, schoolId]
    );
    if (existingUser.rows.length > 0) throw new AppError('User with this email already exists', 409);

    const passwordHash = await hashPassword(parentData.password);
    const sequentialId = await this.generateSequentialId('users');

    const result = await this.executeQuery(
      `INSERT INTO users (first_name, last_name, email, password_hash, role, phone, address, alt_id, school_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id, alt_id, first_name, last_name, email, role, phone, address, is_active, created_at, updated_at`,
      [parentData.firstName, parentData.lastName, parentData.email, passwordHash, 'parent', parentData.phone || null, parentData.address || null, sequentialId, schoolId]
    );
  ...
  private async executeParentsQuery(req: any) {
    ...
    if (isActive !== undefined) { whereClause += ` AND is_active = $${queryParams.length + 1}`; queryParams.push(isActive === 'true'); }
    if (search) {
    ...
    return {
      parents: result.rows.map((parent: any) => this.transformParentResponse(parent)),
  ...
  async getParentById(id: string) {
    ...
    if (parent.role !== 'parent') throw new AppError('User is not a parent', 400);
  ...
  async updateParent(id: string, updateData: UpdateParent) {
    ...
    if (existingParent.role !== 'parent') throw new AppError('User is not a parent', 400);
  ...
  async deleteParent(id: string) {
    ...
    const childrenCheck = await this.executeQuery(
      `SELECT COUNT(*) as children_count FROM student_parents sp JOIN students s ON sp.student_id = s.id WHERE sp.parent_user_id = $1 AND s.is_active = true`,
      [existingParent.id]
    );

    const childrenCount = parseInt(childrenCheck.rows[0].children_count);
    if (childrenCount > 0) {
      throw new AppError(`Cannot delete parent. They have ${childrenCount} active children relationships.`, 409);
    }
  ...
  async linkStudentToParent(linkData: CreateStudentParent) {
    ...
    const parentExists = await this.executeQuery(
      `SELECT id, first_name, last_name FROM users WHERE id = $1 AND school_id = $2 AND role = 'parent' AND is_active = true`,
      [linkData.parentUserId, schoolId]
    );
    if (parentExists.rows.length === 0) throw new AppError('Parent not found or inactive', 404);
    ...
    if (existingRelationship.rows.length > 0) throw new AppError('Relationship between student and parent already exists', 409);
    ...
    return await this.executeTransaction(async (client) => {
      ...
      const result = await client.query(
        `INSERT INTO student_parents (student_id, parent_user_id, relationship_type, is_primary, school_id)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id, student_id, parent_user_id, relationship_type, is_primary, created_at, updated_at`,
        [linkData.studentId, linkData.parentUserId, linkData.relationshipType, linkData.isPrimary || false, schoolId]
      );
  ...
  async unlinkStudentFromParent(studentId: string, parentUserId: string) {
    ...
    if (existingRelationship.rows.length === 0) throw new AppError('Relationship between student and parent not found', 404);
  ...
  async getParentDashboard(parentId: string) {
    ...
    const childrenResult = await this.executeQuery(
      `SELECT s.id, s.student_id as student_number, u.first_name, u.last_name,
       ...
       JOIN users u ON s.user_id = u.id
       JOIN classes c ON s.class_id = c.id
       WHERE sp.parent_user_id = $1 AND s.is_active = true AND s.school_id = $2`,
      [parent.id, schoolId]
    );

    const childrenIds = childrenResult.rows.map((r: any) => r.id);
    let feeSummary = { totalDue: 0, totalPaid: 0, overdueFees: 0 };
    let attendanceSummary: any[] = [];

    if (childrenIds.length > 0) {
      const placeholders = childrenIds.map((_: any, i: number) => `$${i + 2}`).join(', ');

      const feeResult = await this.executeQuery(
        `SELECT COALESCE(SUM(sf.amount), 0) as total_due,
         ...
         WHERE sf.student_id IN (${placeholders}) AND sf.school_id = $1`,
        [schoolId, ...childrenIds]
      );
      ...
      const attResult = await this.executeQuery(
      ...
      attendanceSummary = attResult.rows.map((r: any) => ({
        studentId: r.id,
