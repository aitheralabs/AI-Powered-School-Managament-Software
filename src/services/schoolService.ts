...
export class SchoolService {
  ...
  async updateSubscription(input: UpdateSubscriptionInput): Promise<void> {
    ...
    await query(
      `UPDATE schools SET
         ...
         max_teachers = $6,
         max_staff = $7,
         feature_ai_insights = $8,
         feature_library = $9,
         feature_transport = $10,
         feature_hostel = $11,
         feature_messaging = $12,
         feature_api_access = $13,
         updated_at = NOW()
       WHERE id = $14`,
      [
        input.plan,
  ...
  async exportSchoolData(schoolId: string): Promise<any> {
    const [
      ...
      query(
        "SELECT id, student_fee_id, amount, payment_date, payment_method, transaction_id, receipt_number, created_at FROM payments WHERE school_id = $1",
        [schoolId],
      ),
