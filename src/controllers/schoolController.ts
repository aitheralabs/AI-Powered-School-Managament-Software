...
export const getSchoolUsage = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({
    ...
    data: {
      ...
      usage: {
        students:  { used: parseInt(studentCount.rows[0].count), limit: req.school.limits.maxStudents },
        teachers:  { used: parseInt(teacherCount.rows[0].count), limit: req.school.limits.maxTeachers },
        staff:     { used: parseInt(staffCount.rows[0].count),   limit: req.school.limits.maxStaff },
      },
...
export const listAllSchools = asyncHandler(async (req: Request, res: Response) => {
...
export const reactivateSchool = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
