...
export const linkStudentToParent = asyncHandler(
  async (req: Request, res: Response) => {
    const relationship = await parentService
      .forSchool(req.schoolId!)
      .linkStudentToParent(req.body);
    res
      .status(201)
      .json({
        success: true,
...
export const unlinkStudentFromParent = asyncHandler(
  async (req: Request, res: Response) => {
...
export const getParentChildren = asyncHandler(
  async (req: Request, res: Response) => {
    const result = await parentService
      .forSchool(req.schoolId!)
      .getParentChildren(req.params.parentId);
...
export const updateParentStudentRelationship = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const result = await (svc as any).executeQuery(
      `UPDATE student_parents SET relationship_type = COALESCE($1, relationship_type), is_primary = COALESCE($2, is_primary), updated_at = CURRENT_TIMESTAMP
     WHERE id = $3
     RETURNING *`,
      [
...
export const removeParentStudentRelationship = asyncHandler(
  async (req: Request, res: Response) => {
    const { relationshipId } = req.params;
    const svc = parentService.forSchool(req.schoolId!);
    await (svc as any).executeQuery(
      "DELETE FROM student_parents WHERE id = $1",
      [relationshipId],
    );
...
export const getParentDashboard = asyncHandler(
  async (req: Request, res: Response) => {
    const dashboard = await parentService
      .forSchool(req.schoolId!)
      .getParentDashboard(req.params.parentId);
